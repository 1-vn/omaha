...
def Build(args, omaha_dir, build_all):
  ...
  if build_all:
    mode = 'all'
  command = ['hammer-onevn.bat', 'MODE=' + mode, '--all', '--sha2_authenticode_file=' + key_pfx_path,
...
def PrepareStandalone(args, omaha_dir):
  # copy onevn installer to create standalone installer.
  installer_file = os.path.join(args.root_out_dir[0], args.onevn_installer_exe[0])
  shutil.copyfile(installer_file, os.path.join(omaha_dir, 'omaha', 'standalone', args.onevn_installer_exe[0]))
  ...
  newdata = newdata.replace("ONEVN_INSTALLER_EXE", args.onevn_installer_exe[0])
  ...
  installer_text = "('STANDALONE_FILE_NAME', 'STANDALONE_FILE_NAME', [('ONEVN_VERSION', '$MAIN_DIR/standalone/ONEVN_INSTALLER_EXE', 'APP_GUID')], None, None, None, False, '', '')"
  ...
  installer_text = installer_text.replace("ONEVN_INSTALLER_EXE", args.onevn_installer_exe[0])
  installer_text = installer_text.replace("ONEVN_VERSION", args.onevn_full_version[0])

  # add untagged installer info to standalone_installers.txt.
  untagged_installer_text = "('UNTAGGED_FILE_NAME', 'UNTAGGED_FILE_NAME', [('ONEVN_VERSION', '$MAIN_DIR/standalone/UNTAGGED_INSTALLER_EXE', 'APP_GUID')], None, None, None, False, '', '')"
  ...
  untagged_installer_text = untagged_installer_text.replace("ONEVN_VERSION", args.onevn_full_version[0])

  # add silent installer info to standalone_installers.txt.
  silent_installer_text = "('SILENT_FILE_NAME', 'SILENT_FILE_NAME', [('ONEVN_VERSION', '$MAIN_DIR/standalone/SILENT_INSTALLER_EXE', 'APP_GUID')], None, None, None, False, '', '')"
  ...
  silent_installer_text = silent_installer_text.replace("ONEVN_VERSION", args.onevn_full_version[0])
...
def ParseArgs():
  ...
  parser.add_argument('--root_out_dir',
                      nargs=1)
  parser.add_argument('--onevn_installer_exe',
                      nargs=1)
  ...
  parser.add_argument('--tag_app_name',
                      nargs=1)
  parser.add_argument('--onevn_full_version',
                      nargs=1)
  return parser.parse_args()

def Main(args):
  args = ParseArgs()
  omaha_dir = os.path.join(args.root_out_dir[0], '..', '..', 'onevn', 'vendor', 'omaha')
